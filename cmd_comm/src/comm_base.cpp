...
CommunicatorBase::CommunicatorBase()
    :m_sock(new Socket(Socket::Family::IPv4,Socket::Type::TCP)){

    m_recv_buf.reset(new ByteArray(512));
}
...
void CommunicatorBase::unLock(){
    m_mtx_comm.unlock();
}
bool CommunicatorBase::tryLock(){
    return m_mtx_comm.try_lock();
}
int CommunicatorBase::getClientId(){
    return m_client_id;
}
...
// Message passing 
static auto GetInAddr(struct sockaddr *sa) -> void *{} // get sockaddr, IPv4 or IPv6: 暂时不需要

/// @brief 客户端连接服务器
/// @param node ip地址
/// @param port 端口
/// @return 
SocketPtr CommunicatorBase::connectToServer(const char *node, std::string port){
    
    SYLAR_LOG_INFO(g_logger_sys) << "connnect to server [node:" << node << ", port:" << port<<"]";

    std::vector<IPv4AddressPtr> res;
    IPv4AddressPtr address = IPv4Address::LookupAnyIPAddress(node,Socket::Family::IPv4,Socket::Type::TCP);
...
int CommunicatorBase::sendAll(MsgType &msg_send){
    ...
    // for(int i=0;i<len;i++){
    buf->read(&msg_send[0],len * sizeof(msg_send[0]));
    // }
    size_t byte_left = buf->getSize();
    SocketStream ss(m_sock,false);
    int rt = ss.write(buf,byte_left);
    return rt;
}
...
int CommunicatorBase::sendAll(std::stringstream& msg){
    ...
    int rt = ss.write(&tmp_msg[0],tmp_msg.size());
    return rt;
}
int CommunicatorBase::recvAll(unsigned int sz, ByteArrayPtr buffer){
    size_t tot_bytes = 0;
    size_t n_bytes;
    SocketStream ss(m_sock,false);
    while(tot_bytes < sz){
        n_bytes = ss.read(buffer,sz - tot_bytes);
        if(n_bytes <= 0){
            if(n_bytes == 0){
                SYLAR_LOG_INFO(g_logger_sys) << "------- selectserver: socket hung up. -------";
            }else {
                perror("recv");
                SYLAR_LOG_ERROR(g_logger_sys) << "------- selectserver: socket ERROR! -------";
            }
...
int CommunicatorBase::recvAll(unsigned int sz, MsgType &buffer){
    ...
    while(tot_bytes < sz){
        ...
        n_bytes = ss.read(&buffer[tot_bytes],sz - tot_bytes);
        if(n_bytes <= 0){
            if(n_bytes == 0){
                SYLAR_LOG_INFO(g_logger_sys) << "------- selectserver: socket hung up. -------";
            }else {
...
void CommunicatorBase::writeToBuffer(){
    ...
    for(int i=0;i<ContainerSize;i++){
        ...
        // 将接收到的数据，分离开来
        std::vector<char> tmp_data(m_msgtype_container[i*5]);
        ByteArrayPtr tmp_buf(new ByteArray(m_msgtype_container[i*5])); // 先创建了，不然后面还要再复制一遍
        m_recv_buf->write(&tmp_data[0],m_msgtype_container[i*5]); // 这个读取之后，指针会自己往后移动
        // 主要为了后面不用再拷贝，可以直接使用 bytearray
        tmp_buf->read(&tmp_data[0],m_msgtype_container[i*5]);
        m_buf_recv_data.push_back(tmp_buf); 
    }
}
...
void CommunicatorBase::processBufferOut(){
    ...
    std::list<MessageContainerPtr> mcts;
    {
        ...
        while(cnt < 5 && !m_buf_datas_out.empty()){
            ...
            while(!db->m_lfs.empty()){
                ...
                mct->msg_info.insert(mct->msg_info.end(), lf->m_msgtype.begin(),lf->m_msgtype.end());
    ...
    while(!mcts.empty()){
        ...
        mcts.pop_front();
        if(!rt)SYLAR_LOG_WARN(g_logger_sys) << "send loopframe msg is empty!";
    }
}
...
void CommunicatorBase::processBufferIn(){
    ...
    while(cnt < 5 && checkBufferAndPop()){
        ...
        // 将数据写入
        m_tmp_recv_data->write(tmp_ss,m_tmp_recv_data->getSize());
