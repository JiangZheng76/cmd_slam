 ...
 public:
  T operator()(const F& v) {
    SYLAR_LOG_DEBUG(SYLAR_LOG_ROOT())
        << "from " << typeid(F).name() << " to " << typeid(T).name();
...
struct LoggerDefine {
  ...
  bool operator==(const LoggerDefine& log) const {
    return name == log.name && level == log.level &&
           formatter == log.formatter && appenderIsEqual(log.appender_define);
  }
};
/**
  ...
  LoggerDefine operator()(const std::string& v) {
    ...
    if (node["appenders"].IsDefined()) {
      for (size_t i = 0; i < node["appenders"].size(); i++) {
        ...
        if (!a["type"].IsDefined()) {
          std::cout << "log config appender type is not define " << node
                    << std::endl;
        ...
        if (a["type"].as<std::string>() == "StdoutLogAppender") {
          ...
          std::cout << "log config appender type is define error " << node
                    << std::endl;
  ...
  std::string operator()(const LoggerDefine& v) {
    ...
    for (auto& ap : v.appender_define) {
      ...
      if (ap.type == 1) {
        ...
        std::cout << "log define appender type is define error " << node
                  << std::endl;
 ...
 public:
  std::vector<T> operator()(const std::string& v) {
    ...
    for (size_t i = 0; i < node.size(); i++) {
      ...
      ss << node
              [i];  // node
                    // 写好了通过字符流进行读取的格式，输入进去就变成在yaml文件中看到的格式
      // 调用偏特化将 string 转化成 T
 ...
 public:
  std::map<std::string, T> operator()(const std::string& v) {
    ...
    for (auto it = node.begin(); it != node.end(); it++) {
      ss.str("");
      ss << it->second;
      mp.insert(make_pair(it->first.Scalar(),
...
class ConfigVar : public ConfigVarBase {
 public:
  ...
  std::string toString() override {
    ...
    try {
      // boost::lexical_cast 是一个类型转换工具，它可以将各种基本类型和 STL
      // 类型转换为另一种基本类型或 STL 类型。
      return ToStr()(m_val);
    } catch (std::exception& e) {
      SYLAR_LOG_ERROR(SYLAR_LOG_ROOT())
          << "Config::toString expection." << e.what()
  ...
  bool fromString(const std::string& val) override {
    try {
      ...
      SYLAR_LOG_ERROR(SYLAR_LOG_ROOT())
          << "Config::fromtString expection " << e.what()
...
/**
 * @brief
 * 一个封装的工具类，里面没有非静态成员函数，都是静态成员函数，可以全局随意调用。【获取，设置，添加参数。】
 * @description:
 * @return {*}
 */
class Config {
 public:
  ...
  static void showConfigVar() {
    ...
    for (auto& s_data : GetSdatas()) {
      SYLAR_LOG_INFO(SYLAR_LOG_ROOT())
          << "name = " << s_data.first << ", value = " << s_data.second;
    }
  }
  /**
  ...
  static typename ConfigVar<T>::ptr Lookup(const std::string& name) {
    ...
    // it->second 是 ConfigVarBase::ptr智能指针，将它强制转换为
    // ConfigVar<T>类的指针 相比于 C/C++
    // 中的强制类型转换，std::dynamic_pointer_cast 更加安全
    // 因为它会在运行时检查转换的合法性。
   ...
   * @brief 查找并添加
   * @description:
   * 如果定义了class T 表示 T就是一个类，如果是 typename T
   * 表示不确定T是否是一个类
   * @return {*}
   */
  template <class T>
  static typename ConfigVar<T>::ptr Lookup(
      const std::string name  //  typename 是用于声明 ptr
                              //  是一个类型，而不是一个变量 （必须的）
      ,
    ...
    if (it != GetSdatas().end()) {
      ...
      if (tmp) {
        ...
        SYLAR_LOG_ERROR(SYLAR_LOG_ROOT())
            << "Lookup name = " << name << " but the type is "
    ...
    GetSdatas()[name] = v;
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT())
        << "Lookup name = " << name << " create a new one.";
    return v;
  }
 ...
 private:
   ...
   * 因此，它们不能访问非静态成员变量和非静态成员函数，因为这些变量和函数是与特定对象实例相关联的。？？？（因为静态成员函数编译器不会给他配置
   * this 指针，可以看作一个普通的静态函数） 不知道为什么这个不行？？？static
   * ConfigVarMap s_datas; （也是可以的，封装起来更好管理）
   * @return {*}
   */
