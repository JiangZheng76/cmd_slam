...
class Scheduler {
 public:
  ...
  /// @param thread 调度线程号
  /// @param use_caller
  /// 是否当前调用线程,是否将当前创建该调度器的线程放到调度器里面去，
  // 如果不放作为一个单独管理调度器的线程使用，如果放的话，需要用一个单独的协程来存放这个调度器
  /// @param name 调度器名称
  Scheduler(size_t thread = 1, bool use_caller = true,
  ...
  /// @brief
  /// 批量协程调度，附属的线程默认为-1，批量放入的认识在什么时候处理？？？？从现在来看好像是-1的都不进行处理
 ...
 private:
  ...
  struct FiberAndThread {
    ...
    int thread;
    FiberAndThread(Fiber::ptr fb, size_t thr) : fiber(fb), thread(thr) {}

    FiberAndThread(Fiber::ptr* fb, size_t thr) : thread(thr) {
      fiber.swap(*fb);
    }
    FiberAndThread(std::function<void()> f, size_t thr) : cb(f), thread(thr) {}
    FiberAndThread(std::function<void()>* f, size_t thr) : thread(thr) {
      cb.swap(*f);
    }
