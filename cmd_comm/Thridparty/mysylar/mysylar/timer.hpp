...
class TimerManager {
 ...
 public:
  ...
  Timer::ptr addConditionTimer(
      uint64_t ms, std::function<void()> cb, std::weak_ptr<void> weak_cond,
      bool recurring =
          false);  // 只能指针做条件，智能指针不存在了，就不需要做这个任务了
 ...
 private:
  ...
  bool m_tickle =
      false;  //  防止频繁tickle修改nextimer，如果多个出发，只需要一次tickle就足够了
};
