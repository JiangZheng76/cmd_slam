...
template <class T>
struct TryScopedLockImpl : public Noncopyable {
 public:
  TryScopedLockImpl(T& mutex) : m_mutex(mutex) {}
  ~TryScopedLockImpl() { unlock(); }
  bool trylock() {
    if (!m_locked && m_mutex.trylock()) {
      m_locked = true;
    }
    return m_locked;
  }
...
// 互斥锁
// 上锁的时候释放 cpu 进入到阻塞的状态，锁资源释放之后再进入到就绪状态等待 cpu
// 的调度
class Mutex : public Noncopyable {
 public:
  ...
  typedef TryScopedLockImpl<Mutex> TryLock;
  Mutex() { pthread_mutex_init(&m_lock, nullptr); }
  ~Mutex() { pthread_mutex_destroy(&m_lock); }
  void lock() { pthread_mutex_lock(&m_lock); }
  bool trylock() { return (pthread_mutex_trylock(&m_lock) == 0); }
...
// 自旋锁
// 自旋锁不会释放 cpu 资源，而是会使用 while 循环等待，或者是让 cpu
// 停止执行一会再询问，期间保持对 cpu 的占用
...
// CAS 只是一次操作，去拿资源，比较，可以实现返回 true，不可以实现返回
// false；循环调用 CAS 就可以实现自旋锁的效果
