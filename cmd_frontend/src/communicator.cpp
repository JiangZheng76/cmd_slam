...
namespace cmd
{
    static LoggerPtr g_logger_sys = SYLAR_LOG_NAME("CMD-COMM");
        ...
        :CommunicatorBase()
    {
        ...
        sock->dump(sock_ss);
        SYLAR_LOG_INFO(g_logger_sys) << "connect to backend success " << sock_ss.str();
    ...
    void FrontEndComm::processSendMsgLoopframe()
    {
        ...
        while (!m_buf_msg_out.empty())
        {
            ...
            // SYLAR_LOG_DEBUG(g_logger_sys) << msg->dump();
    ...
    void FrontEndComm::publishMsg(MsgLoopframePtr msg)
    {
        ...
        if (m_process_lfs.size() != 0)
        {
            ...
            while(it!= m_process_lfs.end() &&  num < m_ref_num){
                ...
                SYLAR_LOG_DEBUG(g_logger_sys) << msg->m_twc.inverse() << "\n" << ref->m_twc;
            ...
            SYLAR_LOG_DEBUG(g_logger_sys) << msg->dump();
            // SYLAR_LOG_DEBUG(g_logger_sys) << "processed lf :"  << m_process_lfs.size()<< ", reference loopframe: " << msg->m_ref_cf.size();
