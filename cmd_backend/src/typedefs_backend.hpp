...
namespace cmd
{
	...
	const std::vector<std::vector<precision_t>> col_vec = {
		{0.55, 0.83, 0.78}, {0.98, 0.50, 0.45}, {0.46, 0.32, 0.05}, {1.00, 1.00, 0.70}, {0.75, 0.73, 0.85}, {0.99, 0.71, 0.38}, {0.70, 0.87, 0.41}};
}
...
namespace cmd
{
	...
	using IDPose3Vector = std::vector<std::pair<int, Eigen::Vector3d>,Eigen::aligned_allocator<std::pair<int, Eigen::Vector3d>>>;
	...
	template<typename T>
	using aligned_unordered_map = std::unordered_map<int, T, std::hash<int>, std::equal_to<int>, Eigen::aligned_allocator<std::pair<const int, T>>>;
