...
void generate_spherical_points(
    ...
    const TransMatrixType &cur_cw, double lidar_range,
    std::vector<Eigen::Vector3d> &pts_spherical)
{
    ...
    for (size_t i = 0; i < pts_nearby.size(); i++)
    {
        ...
        // // 大于30帧的距离不要
        if (pts_nearby[i].first + 80 < pts_nearby.back().first)
        {
            continue;
        }

        // new_pts_nearby.push_back({pts_nearby[i].first,pts_nearby[i].second});
