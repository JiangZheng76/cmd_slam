...
namespace cmd
{
    ...
    /// @brief 根据传进来某一帧的点云pts_spherical，创建ringkey和signature用于后面的回环检测
    /// @param pts_spherical
    /// @param ringkey {out}
    /// @param signature {out}
    /// @param lidar_range
                               ...
                               double lidar_range, Eigen::Matrix4d &tfm_pca_rig)
    {
        ...
        for (size_t i = 0; i < pts_aligned.size(); i++) // loop on pts
        {
            ...
            // get projection bin w.r.t. theta and rho,计算pca对齐之后的某一个点属于哪一个格子
        ...
        for (int i = 0; i < num_s_ * num_r_; i++)
        {
            if (max_height(i) >= (-lidar_range))
            {
                ...
                sig_norm_si(i / num_r_) += max_height(i) * max_height(i); // 记录每一个sig的最大高度平方和
            }
        }
