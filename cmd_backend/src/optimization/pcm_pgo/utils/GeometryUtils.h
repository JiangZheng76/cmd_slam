...
struct PoseWithCovariance {
  ...
  explicit PoseWithCovariance(const gtsam::BetweenFactor<T>& between_factor) {
    ...
    // Eigen::Matrix<double,6,6> e1 = Eigen::Matrix<double,6,6>::Identity();
    // Eigen::Matrix<double,6,6> e2 = Eigen::Matrix<double,6,6>::Identity();
    // auto e3 = e1 * e2;
    // std::cout << factor_pointer_cast<gtsam::noiseModel::Gaussian>(
