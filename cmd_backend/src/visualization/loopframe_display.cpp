...
void LoopframeDisplay::setFromLF(LoopframePtr lf) {
  ...
  for (auto p : lf->m_points) {
    if (p.m_idepth_scaled <= 0) continue;
...
void LoopframeDisplay::refreshPC() {
  ...
  for (int i = 0; i < m_num_sparse_points; i++) {
    ...
    tempVertexBuf[i][0] = ((m_original_point2s[i].m_u - cx) / fx /
                           m_original_point2s[i].m_idepth_scaled);
    tempVertexBuf[i][1] = ((m_original_point2s[i].m_v - cy) / fy /
                           m_original_point2s[i].m_idepth_scaled);
    tempVertexBuf[i][2] =
  ...
  if (m_num_gl_buffer_good_points > m_num_gl_buffer_points) {
    ...
    m_vertex_buffer.Reinitialise(pangolin::GlArrayBuffer,
                                 m_num_gl_buffer_points, GL_DOUBLE, 3,
                                 GL_DYNAMIC_DRAW);
  ...
  m_vertex_buffer.Upload(&tempVertexBuf[0],
                         sizeof(precision_t) * 3 * m_num_gl_buffer_good_points,
                         0);
...
void LoopframeDisplay::drawPC(float pointSize) {
  if (!m_buffer_valid || m_num_gl_buffer_good_points == 0) return;
  ...
  // size：指定每个颜色的分量数量。通常为 3（表示 RGB 颜色）或 4（表示 RGBA
  // 颜色）。 type：指定颜色数据的类型。通常使用
  // GL_UNSIGNED_BYTE（每个分量为无符号字节），GL_FLOAT（每个分量为浮点数）等。
  // stride：指定相邻颜色之间的字节偏移量。通常可以设置为
  // 0，表示颜色数据是紧密排列的。
  // pointer：指向颜色数据的指针。这可以是一个数组、缓冲区对象或其他内存位置。
  m_vertex_buffer.Bind();
  glVertexPointer(m_vertex_buffer.count_per_element, m_vertex_buffer.datatype,
                  0, 0);
...
void drawColorCube(float size, const Eigen::Vector3f &color) {
  const float halfSize = size / 2.0f;
  // 顶点坐标
  Eigen::Vector3f vertices[8] = {
      Eigen::Vector3f(-halfSize, -halfSize, -halfSize),
      Eigen::Vector3f(halfSize, -halfSize, -halfSize),
      Eigen::Vector3f(halfSize, halfSize, -halfSize),
      Eigen::Vector3f(-halfSize, halfSize, -halfSize),
      Eigen::Vector3f(-halfSize, -halfSize, halfSize),
...
void LoopframeDisplay::drawPose(float cube_size) {
  // 保存当前模型视图矩阵
  // 保存当前模型视图矩阵
  auto cube_center = m_Twc.translation();
  glPushMatrix();
  Sophus::Matrix4f m = m_Twc.matrix().cast<float>();
  glMultMatrixf((GLfloat *)m.data());
  drawColorCube(cube_size, {m_color[0], m_color[1], m_color[2]});
  // glColor3d(m_color[0], m_color[1], m_color[2]);
  // pangolin::glDrawColouredCube(-(cube_size / 2.0f),
  //  cube_size / 2.0f);  // 参数为正方体边长的一半
  // 恢复之前的模型视图矩阵
  glPopMatrix();
}
