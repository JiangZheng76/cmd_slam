...
FullSystem::FullSystem()
{
	...
	// CMD_MODE
  	comm_ = std::make_shared<DSVComm>("10.0.0.21","8888");
  	comm_->action();
}
...
void FullSystem::printResult(std::string file)
{
	...
	for(FrameShell* s : allFrameHistory)
	{
		...
		myfile << std::setprecision(6) << s->timestamp << 
			std::setprecision(6) << 
			" " << (float)s->camToWorld.translation().x()<<
			" " << (float)s->camToWorld.translation().y()<<
			" " << (float)s->camToWorld.translation().z()<<
			" " << (float)s->camToWorld.so3().unit_quaternion().x()<<
			" " << (float)s->camToWorld.so3().unit_quaternion().y()<<
			" " << (float)s->camToWorld.so3().unit_quaternion().z()<<
			" " << (float)s->camToWorld.so3().unit_quaternion().w() << std::endl;
		std::cout << "test" << std::endl;
	}
	myfile.close();
	std::cout << "save costs to " << file << std::endl;
}
...
void FullSystem::addActiveFrame( ImageAndExposure* image, ImageAndExposure* image_right, int id )
{
	...
	{
		TimeRecord record(&left_selcetpoint_costs);
		fh->makeImages(image->image, &Hcalib);
	}
	
	fh_right->ab_exposure = image_right->exposure_time;
	{
		TimeRecord record(&right_selcetpoint_costs);
